...
pedido.method = {
    ...
    carregarEtapas: (data) => {
        ...
        if (data.idpedidostatus == 6) {
            ...
            let _motivo = '<span class="text mb-0">O estabelecimento recusou seu pedido. Desculpe o transtorno.</span>';

            if (data.motivorecusa != null && data.motivorecusa.trim() != '') {
                _motivo = `<span class="text mb-0"><b>Mensagem: </b>${data.motivorecusa.trim()}</span>`;
            }
        ...
        if (data.idpedidostatus == 2 || data.idpedidostatus == 3) {
            ...
            preparando = preparando.replace(/\${status}/g, 'active')
                .replace(/\${status-icon}/g, '')
                .replace(/\${descricao}/g, 'Seu pedido está sendo separado')
        ...
        if (data.idpedidostatus == 5) {
            ...
            separando = separando.replace(/\${status}/g, '')
                .replace(/\${status-icon}/g, 'status')
                .replace(/\${descricao}/g, '')
        ...
        document.querySelector("#containerAcompanhamento").innerHTML += separando;
    ...
    mensagemWhatsApp: () => {
        ...
        var texto = 'Olá! gostaria de saber sobre meu pedido: Nº ' + idpedido;

        // converte para URL
        let encode = encodeURI(texto);
        let URL = `https://wa.me/5565999664451?text=${encode}`;
