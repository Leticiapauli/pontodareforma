    ...
    init: () => {
        ...
        $("#categoriasMenu").sortable({
            ...
            update: function (event, ui) {
                ...
                catalogo.method.atualizarOrdemCategorias();
            },
        ...
        catalogo.method.carregarListaIcones();
        catalogo.method.obterCategorias();


        // inicializa o drag e drop da imagem
        // Previne os comportamenos padrão do navegador
        ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => {
            DROP_AREA.addEventListener(eventName, catalogo.method.preventDefaults, false)
            document.body.addEventListener(eventName, catalogo.method.preventDefaults, false)
        });

        // Evento quando passa o mouse em cima com a imagem segurada (Hover)
        ['dragenter', 'dragover'].forEach(eventName => {
            DROP_AREA.addEventListener(eventName, catalogo.method.highlight, false)
        });

        // Evento quando sai com o mouse de cima
        ['dragleave', 'drop'].forEach(eventName => {
            DROP_AREA.addEventListener(eventName, catalogo.method.unhighlight, false)
        });

        // Evento quando solta a imagem no container
        DROP_AREA.addEventListener('drop', catalogo.method.handleDrop, false)
...
catalogo.method = {
    ...
    obterCategorias: () => {
        ...
        app.method.get('/categoria',
            (response) => {
                ...
                catalogo.method.carregarCategorias(response.data)
            },
            (error) => {
                app.method.loading(false);
    ...
    carregarCategorias: (lista) => {
        ...
        if (lista.length > 0) {
            ...
            lista.forEach((e, i) => {
                ...
                let temp = catalogo.template.categoria.replace(/\${id}/g, e.idcategoria)
        ...
        if (conteudo <= 0 || forcar) {
            ...
            app.method.get('/produto/categoria/' + idcategoria,
                (response) => {
                    ...
                    catalogo.method.carregarProdutos(response.data, idcategoria)
                },
                (error) => {
                    app.method.loading(false);
                    console.log('error', error)
    ...
    carregarProdutos: (lista, idcategoria) => {
        ...
        if (lista.length > 0) {
            ...
            lista.forEach((e, i) => {
                ...
                let temp = catalogo.template.produto.replace(/\${id}/g, e.idproduto)
                ...
                if ((i + 1) == lista.length) {
                    ...
                    $("#listaProdutos-" + idcategoria).sortable({
                        ...
                        update: function (event, ui) {
                            // função para atualizar a ordem da lista
                            catalogo.method.atualizarOrdemProdutos(idcategoria);
                        },
    ...
    salvarProduto: () => {
        ...
        app.method.post('/produto', JSON.stringify(dados),
            (response) => {
                ...
                catalogo.method.obterProdutosCategoria(CATEGORIA_ID, true);
            },
            (error) => {
                app.method.loading(false);
                console.log('error', error)
    ...
    handleDrop: (e) => {
        ...
        var files = dt.files
        catalogo.method.uploadImagemProduto(files)
    },
    ...
    abrirModalOpcionaisProduto: (idcategoria, idproduto) => {
        ...
        catalogo.method.obterOpcionaisProduto(idproduto);
    ...
    obterOpcionaisProduto: (idproduto) => {
        ...
        app.method.get('/opcional/produto/' + idproduto,
            (response) => {
                ...
                catalogo.method.carregarOpcionaisProduto(response.data);
                catalogo.method.carregarOpcionaisProdutoSimples(response.data);
        ...
        if (lista.length > 0) {
            ...
            Object.entries(listaAgrupada).forEach((e, i) => {
                ...
                for (let index = 0; index < opcional.length; index++) {
                    ...
                    itens += catalogo.template.opcionalItem.replace(/\${idopcionalitem}/g, element.idopcionalitem)
                        .replace(/\${nome}/g, element.nomeopcional)
                        .replace(/\${valor}/g, valor)
                        .replace(/\${idopcional}/g, e[0])
                }

                let temp = catalogo.template.opcional.replace(/\${idopcional}/g, e[0])
    ...
    carregarOpcionaisProdutoSimples: (lista) => {
        ...
        if (listaSimples.length > 0) {
            ...
            document.querySelector("#listaOpcionaisProduto").innerHTML += catalogo.template.opcionalSimples;
            ...
            listaSimples.forEach((e, i) => {
                ...
                let temp = catalogo.template.opcionalItemSimples.replace(/\${idopcionalitem}/g, e.idopcionalitem)
    ...
    changeMinimoMaximoOpcao: () => {
        ...
        catalogo.method.atualizarHintOpcao();
    ...
    salvarOpcional: () => {
        ...
        if (simples) {
            ...
            catalogo.method.salvarOpcionalProduto(dados);
    ...
    salvarOpcionalProduto: (dados) => {
        ...
        app.method.post('/opcional/produto', JSON.stringify(dados),
            (response) => {
                ...
                catalogo.method.abrirModalOpcionaisProduto(CATEGORIA_ID, PRODUTO_ID);
    ...
    adicionarLinhaOpcao: () => {
        ...
        let temp = catalogo.template.opcaoSelecao.replace(/\${id}/g, id);
...
catalogo.template = {
    ...
    categoria: `
        <div class="card mt-3" data-idcategoria="\${id}">
            <div class="card-drag" id="heading-\${id}">
                ...
                <div class="infos">
                    <a href="#!" class="name mb-0" data-bs-toggle="collapse" data-bs-target="#collapse-\${id}" aria-expanded="true" aria-controls="collapse-\${id}" onclick="catalogo.method.obterProdutosCategoria('\${id}')">
                        <span class="me-2">\${icone}</span>
                        <b>\${titulo}</b>
                    </a>
                </div>
                <div class="actions">
                    <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="Editar" onclick="catalogo.method.editarCategoria('\${id}')">
                        <i class="fas fa-pencil-alt"></i>
                    </a>
                    <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="Duplicar" onclick="catalogo.method.abrirModalDuplicarCategoria('\${id}')">
                        <i class="far fa-copy"></i>
                    </a>
                    <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="Remover" onclick="catalogo.method.abrirModalRemoverCategoria('\${id}')">
            ...
            <div id="collapse-\${id}" class="collapse" aria-labelledby="heading-\${id}" data-parent="#categoriasMenu">
                <div class="card-body">
                    ...
                    <div class="card card-select mt-3" onclick="catalogo.method.abrirModalAdicionarProduto('\${id}')">
    ...
    <div class="card mt-3 pl-0" data-idproduto="\${id}">
        <div class="d-flex">
            ...
            <div class="container-img-produto" \${imagem}>
                <a id="btn-remover-imagem-\${id}" onclick="catalogo.method.abrirModalremoverImagemProduto('\${idcategoria}', '\${id}')" href="#!" class="icon-action me-1 mb-1 \${btnRemover}" data-toggle="tooltip" data-placement="top" title="Remover">
                    <i class="fas fa-trash-alt"></i>
                </a>
                <a id="btn-editar-imagem-\${id}" onclick="catalogo.method.abrirModalImagemProduto('\${idcategoria}', '\${id}')" href="#!" class="icon-action me-1 mb-1 \${btnEditar}" data-toggle="tooltip" data-placement="top" title="Editar">
                    <i class="fas fa-pencil-alt"></i>
                </a>
            </div>
            ...
            <div class="actions">
                <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="Opcionais" onclick="catalogo.method.abrirModalOpcionaisProduto('\${idcategoria}', '\${id}')">
                    \${opcionais}
                    <i class="fas fa-layer-group"></i>
                </a>
                <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="Editar" onclick="catalogo.method.editarProduto('\${idcategoria}', '\${id}')">
                    <i class="fas fa-pencil-alt"></i>
                </a>
                <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="Duplicar" onclick="catalogo.method.abrirModalDuplicarProduto('\${idcategoria}', '\${id}')">
                    <i class="far fa-copy"></i>
                </a>
                <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="Remover" onclick="catalogo.method.abrirModalRemoverProduto('\${idcategoria}', '\${id}')">
                    <i class="fas fa-trash-alt"></i>
                </a>
            </div>
        </div>
    ...
    opcaoSelecao: `
        <div class="row linha mt-4" id="opcao-\${id}">
            ...
            <div class="col-1">
                <a href="#!" class="btn btn-red btn-sm mt-4" onclick="catalogo.method.removerLinhaOpcao('\${id}')">
                    <i class="fas fa-trash-alt"></i>
                </a>
            </div>
        </div>
    ...
    opcionalItem: `
        <div class="card card-opcionais mt-2">
            ...
            <div class="checks">
                <div class="actions">
                    <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="Remover" onclick="catalogo.method.abrirModalRemoverOpcionalItem('\${idopcionalitem}')" data-bs-original-title="Remover">
                        <i class="fas fa-trash-alt"></i>
                    </a>
                </div>
            </div>
    ...
    opcionalItemSimples: `
        <div class="card card-opcionais mt-2">
            ...
            <div class="checks">
                <div class="actions">
                    <a href="#!" class="icon-action" data-toggle="tooltip" data-placement="top" title="" onclick="catalogo.method.abrirModalRemoverOpcionalItem('\${idopcionalitem}')" data-bs-original-title="Remover">
                        <i class="fas fa-trash-alt"></i>
                    </a>
                </div>
            </div>
